...
MAX_DATA_SIZE = 100  # Batasi jumlah maksimum data saat menyimpan
...
def train_polynomial_model(token, degree=3):
    ...
    df = price_data.resample('5T').mean()
    df = df.dropna()
    ...
    polynomial_model.fit(X, y)  # Melatih model

    next_time_index = np.array([[len(df)]])
    predicted_price = polynomial_model.predict(next_time_index)[0] 

    # Mengatur fluktuasi harga berdasarkan nilai prediksi
    fluctuation_range = 0.05 * predicted_price  # Menggunakan 5% sebagai interval fluktuasi
    min_price = predicted_price - fluctuation_range
    max_price = predicted_price + fluctuation_range

    # Memilih nilai secara acak dalam kisaran fluktuasi
...
def update_data():
    ...
    for token in tokens:
        download_data(token)
        format_data(token)
        train_polynomial_model(token, degree=3)
